 ...
 *   const { duration, uptime } = timer.measure();
 */
 ...
 * @property {() => {duration:string, uptime:string}} measure
 *   Measure time since the previous call (`duration`) and since `startTime` (`uptime`).
 */
...
const timer = {
   ...
   * - `uptimeFmt`: formatted uptime
   *
   * @returns {{duration:number, uptime:number, durationFmt:string, uptimeFmt:string}}
   */
  measure () {
    let end = now();
    let lag =  now() - end; // ~ 200ns (cold) - ~80ns (warm)
    let duration = end - lastTime - lag;

    if (duration < 0) duration = 0;

    let uptime = end - startTime;
    let durationFmt = format(duration);
    let uptimeFmt = format(uptime);

    let detail = { duration, uptime, durationFmt, uptimeFmt };
